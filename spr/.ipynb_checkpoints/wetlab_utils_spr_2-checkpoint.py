...
def correct_pooled(r, desired_conc, concentration_type = "uM", max_vol=1.5):
    '''
    ...
    if (r.pooled_vol * (r[f'conc_{concentration_type}'] - desired_conc) / desired_conc) + r.pooled_vol > max_vol:
        if (max_vol * desired_conc)/r[f'conc_{concentration_type}'] < 0.02:
            return 0.02
        else:
            return (max_vol * desired_conc)/r[f'conc_{concentration_type}']
    else:
        return ((r.pooled_vol*.8) if max_vol > r.pooled_vol else max_vol)
...
def correct_buffer(r, max_vol=1.5):
    ...
    if r.buffer_vol < 0:
        ...
        return r.buffer_vol
def calculate_dilutions(df, desired_conc, concentration_type = "uM", max_volume=1.5,verbose=True):
    df['pooled_vol_corr'] = df.apply(correct_pooled, args=(desired_conc, concentration_type, max_volume,), axis=1)
...
def gen_ot2_script(
    ...
    for i, r in df.iterrows():
        ...
        pooled_frac_vol_list = [
            float(a) * 1e3
            for a in str(r.pooled_vol_corr).replace("[", "").replace("]", "").split(" ")
            if a != ""
        ]
...
def make_spr_df(df, highest_conc_nM, volume_uL, dilution_step, num_dilutions, checkpoint=False):
    #need to re-calculate normalization
    sample_vol_dilute = (volume_uL/(dilution_step-1))
    max_vol = (sample_vol_dilute)*dilution_step #maximum volume while pipetting
    if max_vol > 200:
        print ("ERROR: wells will overflow during dilution")

    spr_df = pd.DataFrame()
    spr_df["well_id"] = df["well_id"]
    spr_df["conc_uM"] = df["normed_conc_uM"]
    spr_df["pooled_vol"] = df["pooled_vol_corr"] + df["buffer_vol"]
    spr_df["eblock"] = df["eblock"]
    #spr_df["design_aa_seq"] = df["design_aa_seq"]
    spr_df["Vector"] = df["Vector"]
    spr_df["conc_nM"] = spr_df["conc_uM"] * 1000
    spr_df["dilutions"] = num_dilutions
    spr_df["dilution_step"] = dilution_step
    spr_df["volume_step"] = sample_vol_dilute
    spr_df["highest_conc_nM"] = highest_conc_nM
    spr_df.to_csv(f'spr_readin_df_ckpt.csv', index=False)
    print(f'Total samples: {len(spr_df)}')
    return spr_df

def calculate_conc_and_vol_and_drop_samples(spr_df):
    print(f'Before calculating conc and volume: {len(spr_df)} samples')
    max_vol = (spr_df["volume_step"][0])*spr_df["dilution_step"][0] #maximum volume while pipetting
    for i, r in spr_df.iterrows():
        if r.conc_uM < (r.highest_conc_nM/1000)*.99:
            print (f"WARNING! {r.well_id} is at {r.conc_uM*1000} nM. Concentration too low. Skipping well.")
    spr_df = spr_df.drop(spr_df[spr_df.conc_uM < ((spr_df.highest_conc_nM)/1000)*.99].index)
    spr_df.to_csv(f'spr_read_in_checkpoint.csv', index=False)

    calculate_dilutions(spr_df, spr_df["highest_conc_nM"][0], 'nM', (max_vol/1000))

    for i, r in spr_df.iterrows():
        if r.normed_conc_nM > r.highest_conc_nM:
            print (f"WARNING! {r.well_id} cannot be normalized properly. Lowest working concentration is {r.normed_conc_nM} Please dilute samples and retry. Skipping well.")
        if (r.pooled_vol_corr + r.buffer_vol)< (max_vol/1000)*.99:
            sample_vol_dilute = ((r.pooled_vol_corr + r.buffer_vol)/(r.dilution_step))
            vol = (sample_vol_dilute)*(r.dilution_step-1)
            print (f"WARNING! {r.well_id} only has {round((r.pooled_vol_corr + r.buffer_vol)*1000,2)} uL. Amount only allows {vol*1000} uL per well. Skipping well.")

    spr_df = spr_df.drop(spr_df[spr_df.pooled_vol_corr + spr_df.buffer_vol < (max_vol/1000)].index)
    spr_df = spr_df.drop(spr_df[spr_df.normed_conc_nM > spr_df['highest_conc_nM'][0]].index)
    #spr_df['pooled_vol_corr'] = spr_df.apply(correct_pooled, args=((highest_conc/1000), 'uM', (max_vol/1000),), axis=1)
    print(f'After removing samples that cannot be normalized properly: {len(spr_df)} samples')
    return spr_df

def write_spr_csv(spr_df, filename,dest_plate):
    user = getpass.getuser()
    filename = f"{filename}_{user}_spr.csv"
    conc_header = ""
    pos_header = ""
    dilute_string = ""
    dilution = spr_df.normed_conc_nM[0]
    for i in range(spr_df.dilutions[0]):
        conc_header += f",Concentration {i+1}"
        pos_header += f",Position {i+1}"
        dilute_string = f',{round(dilution)}' + dilute_string
        dilution /= spr_df.dilution_step[0]
    header = "plate id,solution" + conc_header + pos_header + "\n"
    if dest_plate == 384:
        plate_format = [[r + str(c) for c in range(1,25)] for r in 'ACEGIKMOBDFHJLNP']
    if dest_plate == 96:
        plate_format = [[r + str(c) for c in range(1,13)] for r in 'ABCDEFG']
    ...
    for w, r in spr_df.set_index("well_id").iterrows():
        pos_string = ""
        well = int(np.floor(((row)/len(plate_format))))*r.dilutions
        #print (f'{plate}_{plate_format[row%len(plate_format)][well]}')
        plate_array.append(plate)
        well_array.append(plate_format[row%len(plate_format)][well])
        for i in range(r.dilutions):
            pos_string = f",{plate_format[row%len(plate_format)][well]}" + pos_string
            well += 1
        row += 1
        if well >= int(dest_plate):
            plate += 1
            well = 0
        line= f"{plate},{w}"+ dilute_string + pos_string + "\n"
        f.write(line)
    spr_df['Destination Plate Name'] = plate_array
    spr_df['Destination Well'] = well_array
    return spr_df

def gen_spr_ot2_script(
    df,
    pool_plate = "384",
    filename="",
    path="",
    date=today,
    template="/net/software/lab/cowboy/SPR_norm_and_dilute.py",
):
    """
    Generate OT-2 script for pooling and normalising SEC fractions.
    ...
    pool_plate: type of plate to pool into, "384" or "96"
    
    path: a path to prepend onto the script output will be in format f"{path}{date}_{filename}{user}_spr_norm_and_dilute.py"

    filename: filename for the generated script. output will be in format f"{path}{date}_{filename}{user}_spr_norm_and_dilute.py"

    date: string of the date that gets prepended to the output files.

    template: path to an OT-2 protocol template.
    """
    
    user = getpass.getuser()
    file_name = f"{path}{date}_{filename}{user}_spr_norm_and_dilute"

    ot2_transfers = ""
    non_normed_transfers = ""
    for i, r in df.iterrows():
        volume = round(r.pooled_vol_corr,6)
        pooled_frac_list = " ".join(
            [
                f"'{a}',"
                for a in str(r.well_id)
                .replace("['", "")
                .replace("']", "")
                .split("' '")
                if a != ""
    ...
    with open(file_name + ".py", "w") as f:
        for l in ot2_template:
            modified_line = l.replace("### PROTOCOL_NAME ###", file_name)
            modified_line = modified_line.replace("### USER_NAME ###", user)
            modified_line = modified_line.replace("### POOL_PLATE ###", str(pool_plate))
            modified_line = modified_line.replace("### TRANSFERS ###", ot2_transfers)
            modified_line = modified_line.replace('### DILUTIONS ###', str(df.dilutions[0]))
            modified_line = modified_line.replace("### STEP ###", str(df.dilution_step[0]))
            modified_line = modified_line.replace("### VOLUME ###", str(df.volume_step[0]))
            f.write(modified_line)
    return