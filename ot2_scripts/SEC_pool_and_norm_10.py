...
def run(protocol: protocol_api.ProtocolContext):
    ...
    if instrument == 'akta':
        ...
        splits = len(transfers[0][0][0].split('-'))
        src_plates = list(sorted(np.unique(['-'.join(x.split('-')[:(splits-1)]) for x in np.hstack(transfers[:, 0])])))
        ...
        for t in transfers:
            ...
            if buffer_vol == 0:
                ...
                while buffer_vol > s300.current_volume:
                    ...
                    s300.dispense(s300.current_volume, pool_plates[des_p][des_w].bottom(4))
                ...
                if buffer_vol <= s300.current_volume:
                    ...
                    s300.dispense(buffer_vol, pool_plates[des_p][des_w].bottom(4))
    ...
    for t in transfers:
        ...
        for frac, vol in list(zip(fractions, frac_vol)):
            ...
            if instrument == 'hplc':
                src_p, src_w = '-'.join(frac.split('-')[:splits-1]), frac.split('-')[splits-1]
            ...
            if frac_ptype == 'greiner_384_wellplate_250ul' or frac_ptype == 'corning_384_wellplate_112ul_flat':
                ...
                if vol <= 50 and buffer_vol > 0:
                    ...
                    s300.move_to(frac_plates[src_p][src_w].bottom(z_off+1.5), speed=0.5)
                elif vol > 50:
